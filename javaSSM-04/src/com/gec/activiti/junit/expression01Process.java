...
public class expression01Process {
    ...
    public void testDeployProcess() throws FileNotFoundException {
        //获取业务对象
        ZipInputStream zipinpt = new ZipInputStream(new FileInputStream("C:\\Users\\徐沛鹏\\Desktop\\bpmn图片icon\\expressionProcess01.zip"));
        Deployment deployment = engine.getRepositoryService()
                .createDeployment()
                .name("表达式分配代办人")
                .addZipInputStream(zipinpt)
                .deploy();
    ...
    public void testStartProcess() {
        String key="expression01Process";
        Map<String, Object> map = new HashMap<>();
        map.put("userId","李来禄"); //session.getAttribute //动态获取
        ProcessInstance pi = engine.getRuntimeService().startProcessInstanceByKey(key,map);
